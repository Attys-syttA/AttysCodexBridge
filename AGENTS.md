...
- `npm run dev` starts AttysCodexBridge with `tsx` against `src/index.ts`.
...
The current history uses short, descriptive commit subjects, for example: `Initial AttysCodexBridge implementation - Telegram bridge for OpenAI Codex CLI SDK`. Keep commit messages imperative, concise, and scoped to one logical change.
