...
This document explains the release automation process used in TelePi and how to reuse it in AttysCodexBridge.

## Goal

Publish AttysCodexBridge from GitHub Actions without storing a long-lived `NPM_TOKEN` secret.
...
Reuse/adapt the same pattern for AttysCodexBridge.

## What AttysCodexBridge needs

AttysCodexBridge is not yet wired for this flow. To adopt it, plan the following:

1. **Choose the package name**
   - recommended: `@attys-sytta/attys-codex-bridge`
   - update `package.json` accordingly
   - add `publishConfig.access = public`

2. **Ensure the package is publishable**
   - add a `files` allowlist
   - add a `bin` entry if AttysCodexBridge should be globally installed as a CLI
...
5. **Configure npm Trusted Publishing**
   - in npm package settings, add a trusted publisher for:
     - repo: `Attys-syttA/AttysCodexBridge`
     - workflow: `.github/workflows/release.yml`
...
## Suggested AttysCodexBridge rollout

1. make AttysCodexBridge npm-publishable
...
Once AttysCodexBridge adopts this setup, document the release flow in:
- `README.md`
- `AGENTS.md`
