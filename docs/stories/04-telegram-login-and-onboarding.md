...
- Persist as little auth state as possible in AttysCodexBridge. Let the Codex CLI remain the source of truth.
...
2. Gate prompt execution so AttysCodexBridge responds with a clear auth-required message instead of failing deep inside the SDK.
...
- AttysCodexBridge can report whether Codex is currently authenticated without requiring the operator to inspect the host manually.
